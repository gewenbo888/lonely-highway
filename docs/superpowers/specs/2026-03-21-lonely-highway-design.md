...
Shenzhen spans ~80km. Unity float precision degrades far from origin. Recenter the world origin when the player moves more than 2km from current origin. Recentering shifts all loaded GameObjects (transforms, rigidbodies, active particle systems, audio sources) in a single frame during `LateUpdate`, after physics has resolved. Performed only when no async tile loads are in flight to avoid race conditions.
...
- Write-to-temp-then-rename pattern to prevent corruption. Keep one previous save as `.bak`
...
- **Minimap** — small corner map showing nearby roads, player position, discovered/undiscovered areas. Garage locations marked. Togglable. Rendered from a pre-baked top-down road texture per tile (generated by pipeline), with a runtime fog-of-war mask that reveals as the player drives
